                    ...
                    <div class="card card-invoice">
                        <div class="card-header ps-3 pe-3 pt-3 pb-0 d-flex-header-table">
                            <div class="div-1-tables-header">
                                <h3 class="card-title">Documentos <badge class="badge"
                        ...
                        <div class="p-0">
                            <div class="main-invoice-list" id="mainInvoiceList">
                                @foreach ($documentos as $key => $documento)
                                    <div class="media view_btn" data-id="{{ $documento->id }}"
                                        data-archivos='{{ $documento->archivos }}' data-numero="{{ $documento->numero }}"
                                        data-tipo="{{ $documento->tipo }}"
                                        data-fecha="{{ date('d/m/Y', strtotime($documento->fecha)) }}">
                                        <div class="media-icon">
                                            <i class="far fa-file-alt"></i>
                                        </div>
                                        <div class="media-body">
                                            <h6><span>DOC-{{ $documento->numero }}</span>
                                                <span>{{ $documento->tipo }}</span>
                                            </h6>
                                            <div>
                                                <p><span>Fecha:</span>
                                                    {{ date('d/m/Y', strtotime($documento->fecha)) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    <div class="card card-invoice">
                        <div class="card-body">
                            <div class="invoice-header">
                                <h1 class="invoice-title">DOCUMENTO</h1>
                                ...
                                <div style="display: flex; justify-content: center;">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item btn_options_factura btnEliminar" href="javascript:void(0)">Eliminar</a>
                                            <a class="dropdown-item btn_options_factura btnComprobante" href="javascript:void(0)">Crear comprobante</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header">
                        <h6 class="modal-title">Añadir documentos</h6>
                        <button aria-label="Close" class="btn-close" data-bs-dismiss="modal" type="button"><span
                                aria-hidden="true">&times;</span></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('documentacion_contabilidad_add') }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            <div class="row row-sm">
                                <div class="col-lg">
                                    <label for="">Tipo</label>
                                    <input type="text" placeholder="Tipo" required class="form-control"
                                        name="tipo_add" id="tipo_add">
                                </div>
                                <div class="col-lg">
                                    <label for="">Fecha</label>
                                    <input type="date" class="form-control" required name="fecha_add" id="fecha_add">
                                </div>
                            </div>
                            <br>
                            <div class="row row-sm">
                                <div class="col-lg">
                                    ...
                                    <input class="form-control" type="file" required name="archivos_add[]" multiple
                                        id="archivos_add">
                                </div>
                            </div>
                            <br>
                            <div class="row row-sm">
                                <div class="col-lg">
                                    ...
                                    <textarea class="form-control" placeholder="Observaciones" name="observaciones_add" id="observaciones_add"
                                        cols="30" rows="10"></textarea>
                                </div>
                            </div>
                            <br>
                            <br>
                            <button type="submit" class="btn btn-primary btn-block" id="btnAdd">Agregar
                                Documento</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="modal-content modal-content-demo">
                    ...
                    <div class="modal-body">
                        <div class="row row-sm">
                            <div class="col-lg">
                                <label for="">Tipo</label>
                                <input type="text" placeholder="Tipo" class="form-control" id="cons_inicio_select">
                            </div>
                        </div>
                        <br>
                        <div class="row row-sm">
                        ...
                        <br>
                        <div class="text-center">
                            <a href="{{ route('documentacion_contabilidad') }}" id="btn_clear">Limpiar filtros</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.view_btn', function() {
                ...
                let archivos = JSON.parse(JSON.stringify($(this).data('archivos')));
                ...
                $('#num_fact_view').text(numero);
                $('#num_2_fact_view').text(tipo);
                $('#compra_view').text(fecha);
                ...
                archivos.forEach(archivo => {
                    $('#productos_view').append(`
                        <tr>
                            <td><a href="images/contabilidad/documentos_contabilidad/${archivo}" target="_blank">${archivo}</a></td>
                        </tr>
                    `);
                });
            });
            ...
            $(document).on('click', '.btnEliminar', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('documentacion_contabilidad_delete') }}",
                            type: 'POST',
                            data: {
                                id: id
                            },
